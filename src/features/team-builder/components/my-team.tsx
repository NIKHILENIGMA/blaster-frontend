            ...
            <section className="max-w-6xl mx-auto">
                <header className="flex flex-col gap-4 mb-6">
                    <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
                        <div className="flex items-center gap-4">
                            ...
                            <div>
                                <h1 className="text-2xl md:text-3xl font-bold">{franchise?.teamName || 'My Franchise'}</h1>
                                <p className="text-gray-500 text-sm max-w-md">
                                    Your active cycle squad contains {players.length} players. Build the lineup for the next fixture from this roster.
                                </p>
                            </div>
                        </div>
                        ...
                        <div className="flex flex-wrap gap-2">
                            ...
                            <Button
                                variant="secondary"
                                onClick={() => nextFixture && navigate(`/my-squad/${nextFixture.id}/change`)}
                                disabled={!nextFixture}>
                                Manage Playing 12
                            </Button>
                            <Button
                                variant="outline"
                                onClick={() => nextFixture && navigate(`/my-squad/${nextFixture.id}/roles`)}
                                disabled={!nextFixture}>
                                Change Roles
                            </Button>
                        </div>
                    ...
                    {nextFixture ? (
                        <div className="rounded-xl border border-border bg-card px-4 py-3 text-sm">
                            Next fixture: <strong>{nextFixture.teamA}</strong> vs <strong>{nextFixture.teamB}</strong> at{' '}
                ...
                {players.length === 0 ? (
                    <div className="flex items-center justify-center h-64">
                        <p className="text-gray-500">No squad saved yet. Build your 25-player roster first.</p>
                    </div>
                ) : (
                    <TeamSection
